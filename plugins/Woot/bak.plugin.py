...
class Woot(callbacks.Plugin):
    ...
    def _woot_wootoff(self, irc):
        """woot! wootoff determination (test only)"""
        soup = self._woot_soupcontent(irc, "http://www.woot.com")
        if soup:
            try:
                import re
                wootoff = soup.find(text=re.compile("isWootOff"))
                p = re.compile(r'[\w\W]+var\ isWootOff\ =\ (\w+)')
                wootoff = p.search(wootoff).group(1)
                if (wootoff == 'false'):
                    return False
                else:
                    wootoffpct = soup.find("div", {'class':'bar'})
